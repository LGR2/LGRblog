...
It's clear that girls outperform boys. To quantify how much better girls perform, we'll analyse the data. We need to consider what type of data we are working with. Our two variables are grades and gender, so we'll use a contingency table. The question we want to answer is whether there is a difference in achievement between the sexes. The chi-squared test tells us how much the observed data (the actual data) differs from what we would expect if there were no association between the variable, i.e., gender has no effect on grade achieved—this is our null hypothesis. 
$$\chi ^{2}= \sum \frac{(O_{i}-E_{i})^{^{2}}}{E_{i}}$$
Unfortunately, the spreadsheet with all the original data is formatted in an awful way, so we'll have to construct out contingency table ourselves. 
...
The high X-squared value and very low p-value confirms what we could see visually from the graph: the results between boys and girls are not independent. We now want to determine the size of the effect of sex. By itself, the chi-squared test does not tell us how large the effect of sex has on grades. To do this, we'll use Cramer's V to determine the effect size. Cramer's V ranges from 0 to 1, where 0 indicates no assocaition and 1 indicates a perfect association, where n is the sample or population size, r is the number of rows, and c is the number of columns in the contingency table.

$$\phi_{Cramer's V} = \sqrt{\frac{\chi^{2}}{n\cdot min(r-1);(c-1)}}$$

Additionally, to provide some more rigour, we'll also calculate the mean grades by assigning a value to each grade (1 for H1 and 8 for H8), and calculate the standard deviation.
...
This was just a fun little project. But it also raises some important questions. Why are boys falling so far behind? Why are girls still weaker at Mathematics? What can we do to address these issues?
